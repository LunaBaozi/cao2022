...
score_df = pd.read_csv(args.fast_cart_fastrelax_out, sep="\s+")
...
print("We have %i datapoints with KD and counts. We gathered %i positions from pdbs (%i total mutations)."%
                                                                            (len(kd_df), len(pose_df), len(pose_df)*20 ))
save_kd_df = kd_df
...
one_of_each_mask = np.zeros(len(df), np.bool)
